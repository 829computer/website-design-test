...
const musicCategories = {
  albums: { label: "专辑", en: "Albums" },
  singles: { label: "单曲", en: "Singles" },
...
function pathFor(locale: Locale, section: Section = "home") {
  const localeRoot = `/artist/${locale}`;
  if (section === "home") {
    return localeRoot;
  }
...
function FollowLinksSection({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="border-t border-[#111111] pt-10">
      <div className="mb-8 flex items-end justify-between gap-6">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-[#111111]/45">FOLLOW US ON</p>
          <h3 className="mt-4 font-serif text-4xl leading-none md:text-6xl">Follow us on</h3>
        </div>
        <p className="max-w-sm text-right text-sm leading-6 text-[#111111]/54">
          {isEnglish
      ...
      <div className="space-y-10">
        {followGroups.map((group) => (
          <div key={group.title} className="border-y border-[#111111]">
            <div className="grid gap-5 py-6 md:grid-cols-[0.28fr_1fr]">
              <div>
                {!isEnglish && (
                  <p className="text-xs uppercase tracking-[0.28em] text-[#111111]/45">{group.eyebrow}</p>
                )}
                <h4 className={`${isEnglish ? "" : "mt-3"} font-serif text-3xl`}>
              ...
              <div className="divide-y divide-[#111111]">
                {group.links.map((item) => (
                  <a
                    ...
                    href={item.href}
                    className="group grid gap-4 py-5 transition-colors hover:bg-[#111111] hover:px-5 hover:text-[#f4f0e8] sm:grid-cols-[1fr_auto] sm:items-center"
                  >
                    <span className="flex items-center gap-4">
                      <span className="grid size-12 place-items-center border border-current">
                        <PlatformLogo icon={item.icon} />
                      </span>
                      <span>
                        <span className="block text-2xl font-semibold leading-tight">{item.name}</span>
                        <span className="mt-1 block text-sm leading-6 text-current/58">{item.note}</span>
                      </span>
                    </span>
...
function BusinessContactBlock({ compact = false, locale = "en" }: { compact?: boolean; locale?: Locale }) {
  ...
  return (
    <section className="border-y border-[#111111] py-8">
      {!isEnglish && <p className="text-xs uppercase tracking-[0.3em] text-[#111111]/45">BUSINESS CONTACT</p>}
      <h3 className={`${isEnglish ? "" : "mt-4"} font-serif text-4xl leading-none md:text-6xl`}>
        {isEnglish ? "BUSINESS CONTACT" : "商务联系"}
      </h3>
      <div className="mt-8 divide-y divide-[#111111] border-y border-[#111111]">
        <a
          href={`mailto:${businessEmail}`}
          className="group flex items-center justify-between gap-6 py-5 transition-colors hover:bg-[#111111] hover:px-5 hover:text-[#f4f0e8]"
        >
          <span>
            <span className="block text-xs uppercase tracking-[0.24em] text-current/45">Email / {labels.business}</span>
            <span className="mt-2 block text-2xl font-semibold">{businessEmail}</span>
          </span>
        ...
        >
          <span>
            <span className="block text-xs uppercase tracking-[0.24em] text-current/45">Phone / {labels.phone}</span>
            <span className="mt-2 block text-2xl font-semibold">{businessPhoneDisplay}</span>
          </span>
          <ArrowUpRight className="size-5 shrink-0 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
        </a>
      </div>
...
function PlatformLinks({ title, items }: { title: string; items: string[] }) {
  return (
    <section className="border-t border-[#111111] pt-6">
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-2">
        {items.map((item) => (
          <a
            key={item}
            href="#todo"
            className="group flex items-center justify-between border border-[#111111] px-4 py-4 text-sm tracking-[0.12em] transition-colors hover:bg-[#111111] hover:text-[#f4f0e8]"
          >
            {item}
            <ArrowUpRight className="size-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
          </a>
...
function NewsletterBlock({ locale }: { locale: Locale }) {
  const copy = getCopy(locale);
  return (
    <section className="bg-[#111111] px-5 py-16 text-[#f4f0e8] md:px-8 md:py-24">
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="relative mt-10 overflow-hidden">
            <iframe
              ...
              title="Join the Yuyu Li Music release list"
              className="min-h-80 w-full bg-transparent"
            />
            <div aria-hidden="true" className="absolute right-0 top-[12rem] z-10 h-16 w-52 bg-[#111111]" />
          </div>
        </div>
      </div>
    </section>
    ...
    <nav className="top-32 h-fit space-y-7 text-xl font-semibold md:sticky" aria-label="Music categories">
      ...
      {getMusicCategoryKeys().map((category) => {
        ...
        return (
          ...
          >
            {musicCategories[category].label}
          </Link>
        );
      })}
    </nav>
          ...
          {visibleWorks.map((work) => (
            <Link key={work.slug} href={musicPathFor(locale, work.slug)} className="group block">
              ...
              <div className="pt-5">
                ...
                <h3 className="mt-2 text-2xl font-semibold leading-8 transition-colors group-hover:text-[#8f6d16]">
                  {work.title}
                </h3>
                <p className="mt-1 text-sm tracking-[0.16em] text-[#111111]/45">
                  {work.artist} / {work.typeLabel}
                </p>
              </div>
            </Link>
          ))}
...
function StreamingLinksBlock() {
  return (
    <section className="border-t border-[#111111] pt-8">
      <h3 className="font-serif text-4xl">数位收听</h3>
      <div className="mt-8 divide-y divide-[#111111]/18 border-y border-[#111111]/18 bg-white/38">
        {streamingLinks.map((item) => (
          <a
            key={item.name}
            href={item.href}
            className="group flex items-center justify-between gap-6 px-4 py-4 transition-colors hover:bg-[#111111] hover:text-[#f4f0e8]"
          >
            <span className="flex items-center gap-4">
            ...
            <span className="rounded bg-[#e9edf3] px-4 py-3 text-sm font-bold text-[#111111]">Play</span>
          </a>
        ))}
      </div>
    </section>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {primarySocialLinks.map((item) => (
          <a
            key={item.name}
            href={item.href}
            className="group grid size-12 place-items-center border border-[#111111] transition-colors hover:bg-[#111111] hover:text-[#f4f0e8]"
            aria-label={item.name}
          >
            <PlatformLogo icon={item.icon} />
          </a>
...
function VideoWall({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-20 md:px-8 md:py-28">
      <div className="border-t-4 border-[#b99a45] pt-8">
        <div className="mb-10 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-8 text-[#111111]/68">
              MV、现场、歌词影片和制作片段会平铺在这里；当前封面为模拟占位，后续替换真实 YouTube 缩略图或官方剧照。
            </p>
          </div>
          <p className="text-xs uppercase tracking-[0.28em] text-[#111111]/45">YouTube linked preview grid</p>
        </div>
        ...
        <div className="grid gap-7 sm:grid-cols-2 xl:grid-cols-3">
          {videoItems.map((item, index) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="group grid overflow-hidden bg-[#2f2f2d] text-[#f4f0e8] transition-transform duration-300 hover:-translate-y-1"
            >
              <VideoThumbPlaceholder tone={item.tone} index={index} />
              <div className="relative min-h-40 border-l-4 border-[#d6bd73] px-5 py-5">
                <div className="flex items-center justify-between gap-4">
                  <p className="text-sm font-semibold tracking-[0.08em] text-[#d6bd73]">{item.date}</p>
                  <span className="grid size-7 place-items-center rounded-full bg-[#d6bd73] text-[#111111]">
                    <Play className="size-3 fill-current" />
                  </span>
                </div>
                <p className="mt-2 text-sm tracking-[0.12em] text-[#f4f0e8]/58">{item.artist}</p>
                <h3 className="mt-2 text-xl font-semibold leading-7">{item.title}</h3>
                <span className="mt-5 inline-flex items-center gap-2 text-xs uppercase tracking-[0.22em] text-[#f4f0e8]/62">
                  YouTube
                  <ArrowUpRight className="size-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
                </span>
              </div>
...
function WritingPreviewVisual({ tone, index }: { tone: string; index: number }) {
  const accent = tone === "ink" ? "#f4f0e8" : tone === "paper" ? "#111111" : "#d6bd73";
  const background = tone === "ink" ? "#111111" : tone === "paper" ? "#e8dfcf" : "#2b2417";
            ...
            <>
              <div className="border-t border-[#111111] pt-6">
                <p className="text-xl leading-10 text-[#111111]/74">
                  TODO: 这里放音乐人简介、创作方向、重要经历、媒体简介和照片资产。当前先保留结构，后续替换为正式文本。
                </p>
              </div>
...
export function MusicWorkPage({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  return (
    <main className="min-h-screen bg-[#eef1f7] text-[#111111]">
      <ArtistHeader locale={locale} section="music" />
      <section className="px-5 pb-20 pt-44 md:px-8 md:pb-28">
        <div className="mx-auto max-w-7xl">
          <h1 className="font-serif text-5xl leading-none md:text-7xl">音乐</h1>
            ...
            <div className="grid gap-14 lg:grid-cols-[0.52fr_0.48fr] lg:items-start">
              ...
              <div className="space-y-10">
                <div>
                  ...
                  <h2 className="mt-5 text-4xl font-black leading-tight md:text-5xl">{work.title}</h2>
                  <p className="mt-8 text-2xl leading-9 text-[#111111]/72">{work.date} 发行.</p>
                </div>
                <StreamingLinksBlock />
                <section className="border-t border-[#111111] pt-8">
                  <div className="flex items-end justify-between border-b-4 border-[#111111] pb-6">
                    <h3 className="font-serif text-4xl">曲目</h3>
